...
//D3DFVF_XYZ 和 D3DFVF_XYZRHW的区别是有没有经过坐标变换，如果要坐标变换就用原始坐标
//-----------------------------------【全局变量声明部分】-------------------------------------
//	描述：全局变量的声明
//------------------------------------------------------------------------------------------------
LPDIRECT3DDEVICE9 g_pd3dDevice = NULL;			//direct3D设备对象
ID3DXFont* g_pFont = NULL;						// 字体接口
float g_Fps = 0.0f;								// 帧率
wchar_t g_strFps[50];							// 帧率的字符串数组
LPDIRECT3DVERTEXBUFFER9 g_pVertexBuffer = NULL;	// 顶点缓存对象
LPDIRECT3DINDEXBUFFER9  g_pIndexBufer = NULL;	// 索引缓存对象
...
HRESULT Objects_Init(HWND hwnd)
{
	...
	srand(timeGetTime());													// 用时间作为随机数种子timeGetTime()返回ms级时间

	// 顶点索引缓存四部曲之二 创建顶点合索引缓存
	// 创建顶点缓存
...
void Direct3D_Render(HWND hwnd)
{
	...
	// 获取键盘消息并给予设置相应的填充模式
	if (::GetAsyncKeyState(VK_UP))         // 若数字键1被按下，进行线框填充
		g_pd3dDevice->SetRenderState(D3DRS_FILLMODE, D3DFILL_WIREFRAME);
	if (::GetAsyncKeyState(VK_DOWN))         // 若数字键2被按下，进行实体填充
		g_pd3dDevice->SetRenderState(D3DRS_FILLMODE, D3DFILL_SOLID);				// 设置为线填充
	
	g_pd3dDevice->SetStreamSource(0, g_pVertexBuffer, 0, sizeof(CUSTOMVERTEX));				// 设置渲染流水线
	g_pd3dDevice->SetFVF(D3DFVF_CUSTOMVERTEX);								// 指定FVF为我设定的宏格式
	g_pd3dDevice->SetIndices(g_pIndexBufer);					// 设定索引缓存
	g_pd3dDevice->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0, 0, 8, 0, 12);	// 绘制
...
VOID			Matrix_Set()
{
	...
	D3DXVECTOR3 vEye(0.0f, 0.0f, -200.0f);			// 定义摄像机位置
	D3DXVECTOR3 vAt(0.0f, 0.0f, 0.0f);				// 定义观察点的位置
	D3DXVECTOR3 vUp(0.0f, 1.0f, 0.0f);				// 定义向上的向量
